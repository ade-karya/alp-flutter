...
class FirebaseSyncService {
  ...
  Future<void> syncAssignments({
    ...
    try {
      await _firestore.collection('sync_sessions').doc(sessionCode).update({
  ...
  Future<List<Map<String, dynamic>>> getAssignments(String sessionCode) async {
    try {
      final sessionDoc = await _firestore
          .collection('sync_sessions')
          .doc(sessionCode)
  ...
  Future<bool> submitAnswer({
    ...
    try {
      final submissionId =
          '${studentId}_${assignmentId}_${DateTime.now().millisecondsSinceEpoch}';
  ...
  Stream<List<Map<String, dynamic>>> listenToSubmissions(String sessionCode) {
    return _firestore
        .collection('sync_sessions')
        .doc(sessionCode)
        .collection('submissions')
  ...
  Stream<Map<String, dynamic>> listenToConnectedStudents(String sessionCode) {
    return _firestore
        .collection('sync_sessions')
        .doc(sessionCode)
        .snapshots()
  ...
  /// Generate 6-digit session code using cryptographic random
